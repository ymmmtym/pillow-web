...
def test_invalid_mode_cmyk(client: FlaskClient) -> None:
    rv = client.get("/test?mode=CMYK")
    assert rv.status_code == 400


def test_invalid_mode_empty(client: FlaskClient) -> None:
    rv = client.get("/test?mode=")
    assert rv.status_code == 400


def test_invalid_mode_l(client: FlaskClient) -> None:
    rv = client.get("/test?mode=L")
    assert rv.status_code == 400
